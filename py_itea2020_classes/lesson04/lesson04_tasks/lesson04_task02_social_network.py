...
class Authorization:
    ...
    def check_login_is_available(self, user):
        result = True
        try:
            if self.__class__._registered_users[user]:
                result = False
        except KeyError:
            result = True
        return result
    ...
    def register(self, user, password, password_confirmation, is_admin):
        if len(user) == 0 or len(password) == 0 or len (password_confirmation) == 0:
            ...
            self.__class__._registered_users[user] = {'password': password,
    ...
    def login(self, user, password):
        try:
            if self.__class__._registered_users[user]['password'] == password:
                self._is_authorized = True
                self._current_user = user
                self._current_user_is_admin = self.__class__._registered_users[user]['is_admin']
    ...
    def print_users_info(self):
        if self._is_authorized:
            if self._current_user_is_admin:
                print('***Users Report***')
                for u in Authorization._registered_users.keys():
                    print(f"    User name: {u}, registered: {str(Authorization._registered_users[u]['registration_date'])}")
